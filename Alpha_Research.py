...
class Alpha_Research():
    def __init__(self, data_path="data", file_name="data.sav"):
        ...
        # only for use of formulaic alpha
        self.d = datas[0]
        self.m = {**datas[1], **datas[2]}
    ...
    def data_help(self):
        print("Daily: ", list(self.data_daily.keys()))
        print("Monthly Accounting: ", list(self.data_monthly_acc.keys()))
        print("Monthly Market: ", list(self.data_monthly_mkt.keys()))
    
    def neutralize(self, x):
        demean = x - x.mean()
        return demean / abs(demean).sum()
    ...
    def generate_weight(self, features, operation=None, trunc=0.05):
        if type(features) == pd.core.frame.DataFrame:
            features = [features]
        else:
            pass
